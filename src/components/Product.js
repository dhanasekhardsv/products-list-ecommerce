...
const Product = ({ id, prod }) => {
    ...
    return (
        <>
            <img src={dragIcon} alt="drag-icon" width={10} height={15} className='drag-icon prod cursor-pointer' />
            ...
            <div className='product-details prod'>
                ...
                <img src={editIcon} alt="Edit" className='prod-edit cursor-pointer' height={20} width={15} onClick={() => handleEdit(prod.id)} />
            </div>
            {
                discountEnabled && (
                    <>
                        <input type="number" max={100} name="discount-qty" id={"discount-qty-" + id} className='discount-qty prod' />
                        <select name="discount-type" id={"discount-type" + id} className='discount-type prod cursor-pointer'>
                            <option value="percentage-off">% Off</option>
                            <option value="flat-off">Flat Off</option>
                        </select>
                        <img src={crossIcon} alt="Close" className='remove-prod cursor-pointer' width={15} height={15} onClick={removeProd} />
                    </>
                )
            }
            {
                !discountEnabled && <button type="button" className='add-discount-btn prod cursor-pointer' onClick={() => setDiscountEnabled(true)}>Add Discount</button>
            }
            {
                prod.variants && prod.variants.length && (
                    <>
                        <div className='variant-label'>
                            <span onClick={showHideVariants} className='variant-show-label cursor-pointer'>{showVariants ? 'Hide' : 'Show'} variants</span>
                                    ...
                                    return (
                                        <React.Fragment key={variant.id}>
                                            <div className='product-details prod-variant'>
                                                <img src={dragIcon} alt="drag-icon" width={10} height={15} className='drag-icon prod-variant cursor-pointer' />
                                            ...
                                            </div>
                                            {
                                                discountEnabled && (
                                                    <>
                                                        ...
                                                        <select name="discount-type" id={"discount-type" + variant.id} className='discount-type prod-variant cursor-pointer'>
                                                            <option value="percentage-off">% Off</option>
                                                            <option value="flat-off">Flat Off</option>
                                                        </select>
                                                        <img src={crossIcon} alt="Close" className='remove-prod prod-variant cursor-pointer' width={15} height={15} onClick={() => removeProdVariant(prod, variant.id)} />
                                                    </>
                                                )
                                            }
                                            {
                                                !discountEnabled && <button type="button" className='add-discount-btn prod-variant cursor-pointer' onClick={() => setDiscountEnabled(true)}>Add Discount</button>
                                            }
                                        </React.Fragment>
                                    )
                                })
